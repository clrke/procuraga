...
def pperunit(table_name, year,limit):
	return "SELECT org_name, publish_date, business_category, item_name, item_description, budget,qty,region, province FROM {0} WHERE bid.ref_id = item.ref_id and org.org_id = bid.client_agency_org_id ORDER BY publish_date desc LIMIT {2}".format(table_name,year,limit)
